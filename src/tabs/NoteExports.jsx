...
function formatPropValue(propDef, raw) {
  ...
  if (propDef?.name === 'hs_note_body') {
    return String(raw).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()
  }
  return String(raw)
}

function noteHsUrl(note) {
  const deal = note.deals?.[0]
  const contact = note.contacts?.[0]
  const company = note.companies?.[0]
...
export default function NoteExports() {
  ...
  // Raw HubSpot property columns (note body first), then derived association columns
  const propColumns = useMemo(() => {
    const body = properties.find(p => p.name === 'hs_note_body')
    const rest = properties.filter(p => p.name !== 'hs_note_body')
    return [...(body ? [body] : []), ...rest].map(p => ({
      key: p.name,
      label: p.label || p.name,
      wide: p.name === 'hs_note_body',
      get: note => formatPropValue(p, note.properties?.[p.name]),
    }))
  }, [properties])

  const derivedColumns = useMemo(() => [
    { key: '_owner', label: 'Owner', get: n => n.ownerName || '' },
    {
    ...
    { key: '_contactId', label: 'Contact ID', get: n => n.contacts?.[0]?.id || '' },
    { key: '_deal', label: 'Associated Deal', get: n => n.deals?.[0]?.dealname || '' },
    { key: '_dealId', label: 'Deal ID', get: n => n.deals?.[0]?.id || '' },
    { key: '_company', label: 'Associated Company', get: n => n.companies?.[0]?.name || '' },
    { key: '_companyId', label: 'Company ID', get: n => n.companies?.[0]?.id || '' },
    {
    ...
    { key: '_leadId', label: 'Lead ID', get: n => n.leads?.[0]?.id || '' },
  ], [])
  ...
  const filtered = useMemo(() => {
    ...
    return notes.filter(note =>
      columns.some(col => String(col.get(note) || '').toLowerCase().includes(q))
    )
  }, [notes, columns, search])
  ...
  function exportToExcel() {
    ...
    const rows = filtered.map(note => columns.map(c => c.get(note)))
    const stamp = new Date().toISOString().split('T')[0]
    downloadCsv(`hubspot-notes-${stamp}.csv`, [header, ...rows])
  }

  if (loading && !notes.length) {
    return <div className="state-box">Loading notes…</div>
  }

  if (error) {
    return <div className="state-box error">Error loading notes: {error}</div>
  }
    ...
    <div className="panel">
      <div className="panel-header">
        <div>
          ...
          <div className="panel-sub">
            {lastLoadedStr ? `Loaded at ${lastLoadedStr}` : 'Every HubSpot note, all fields'}
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {pageRows.map(note => {
              const url = noteHsUrl(note)
              return (
                <tr key={note.id}>
                  {columns.map(col => {
                    const val = col.get(note)
                    return (
                      <td
                        key={col.key}
      ...
      <div className="pagination">
        <span className="pagination-info">
          Showing {filtered.length === 0 ? 0 : start + 1}–{Math.min(start + PAGE_SIZE, filtered.length)} of {filtered.length} notes
        </span>
