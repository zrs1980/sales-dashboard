...
export default function CebaServicesPipeline({ data, loading }) {
  if (loading && !data) return <div className="state-box">Loading CEBA Services pipeline…</div>
  if (!data) return null
  ...
  const weighted = open.reduce((s, d) => {
    ...
    const stage = (stageMap && stageMap[p.dealstage]) || getStageLabel(p.dealstage)
    const prob = parseFloat(p.hs_deal_stage_probability || getStageProb(stage))
    return s + parseFloat(p.amount || 0) * prob
  }, 0)
    ...
    <>
      ...
      <div className="panel">
        <div className="panel-header">
          <div>
            <div className="panel-title">Open CEBA Services Deals</div>
