...
function callHsUrl(call) {
  const deal = call.deals?.[0]
  const contact = call.contacts?.[0]
  const company = call.companies?.[0]
...
export default function CallExports() {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const res = await fetch('/api/all-calls')
  ...
  // Raw HubSpot property columns (call title first), then derived association columns
  const propColumns = useMemo(() => {
    const title = properties.find(p => p.name === 'hs_call_title')
    const rest = properties.filter(p => p.name !== 'hs_call_title')
    return [...(title ? [title] : []), ...rest].map(p => ({
      key: p.name,
      label: p.label || p.name,
      wide: isBodyField(p.name),
      get: call => formatPropValue(p, call.properties?.[p.name]),
    }))
  }, [properties])
  ...
  const filtered = useMemo(() => {
    ...
    return calls.filter(call =>
      columns.some(col => String(col.get(call) || '').toLowerCase().includes(q))
    )
  }, [calls, columns, search])
  ...
  function exportToExcel() {
    ...
    const rows = filtered.map(call => columns.map(c => c.get(call)))
    const stamp = new Date().toISOString().split('T')[0]
    downloadCsv(`hubspot-calls-${stamp}.csv`, [header, ...rows])
  }

  if (loading && !calls.length) {
    return <div className="state-box">Loading calls…</div>
  }

  if (error) {
    return <div className="state-box error">Error loading calls: {error}</div>
  }
    ...
    <div className="panel">
      <div className="panel-header">
        <div>
          ...
          <div className="panel-sub">
            {lastLoadedStr ? `Loaded at ${lastLoadedStr}` : 'Every HubSpot call, all fields'}
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {pageRows.map(call => {
              const url = callHsUrl(call)
              return (
                <tr key={call.id}>
                  {columns.map(col => {
                    const val = col.get(call)
                    return (
                      <td
                        key={col.key}
            ...
            {pageRows.length === 0 && (
              <tr>
                <td colSpan={columns.length + 1} style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 40 }}>
                  No calls match the current search
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="pagination">
        <span className="pagination-info">
          Showing {filtered.length === 0 ? 0 : start + 1}–{Math.min(start + PAGE_SIZE, filtered.length)} of {filtered.length} calls
        </span>
